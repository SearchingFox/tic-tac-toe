...
renderBoard b = map (\(x, y) -> case x of 
...
makeMove gs pos = GameState newBoard (nextMark mk) where
    b = board gs
    mk = activeMark gs
    newBoard = let (x,y:ys) = splitAt pos b in case y of 
...
isWin b m = (any . all) (Just m ==) $ rows ++ cols ++ diag where
