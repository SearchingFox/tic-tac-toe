...
endHandler result respond = do
    ...
    let ls = lines file
    let rendered = concat $ take 9 ls ++ ["<h1>", (if result == "draw" then "Ничья!" else T.unpack result ++ " выиграл!"), "</h1>",
                    "<a href=\"/restart\"><button class=\"btn\">Новая игра</button></a>"] ++ drop 14 ls
